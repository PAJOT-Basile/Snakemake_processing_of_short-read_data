...
tryCatch(
    expr = {
        ...
        library(anyLib)
        anyLib(c("tidyverse", "ggpubr", "argparse"))
    },
    error = function(e){
        install.packages(setdiff(c("tidyverse", "ggpubr", "argparse"), rownames(installed.packages())))
        library(tidyverse); library(ggpubr); library(argparse)
    }
)
